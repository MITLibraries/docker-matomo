...
class CustomReportsDao
{
    ...
    /**
     * Maximum length of the serialized `multiple_idsites` column. Kept in sync with the column
     * definition in {@see install()} so the storage boundary lives in exactly one place.
     */
    public const MULTIPLE_IDSITES_MAX_LENGTH = 2000;
    ...
    public function install()
    {
        ...
        DbHelper::createTable($this->table, "
                  ...
                  `multiple_idsites` VARCHAR(" . self::MULTIPLE_IDSITES_MAX_LENGTH . "),
    ...
    /**
     * Serializes website IDs for the `multiple_idsites` column, rejecting a value that exceeds the
     * column length.
     *
     * @param int[]|string[] $multipleIdSites
     * @return string
     * @throws Exception when the serialized value exceeds the column length.
     */
    public function serializeMultipleIdSites(array $multipleIdSites): string
    {
        $serialized = implode(',', $multipleIdSites);

        if (strlen($serialized) > self::MULTIPLE_IDSITES_MAX_LENGTH) {
            throw new Exception(Piwik::translate('CustomReports_ErrorXTooLong', ['multipleIdSites', self::MULTIPLE_IDSITES_MAX_LENGTH]));
        }
    ...
    public function addCustomReport($idSite, $name, $description, $reportType, $dimensions, $metrics, $segmentFilter, $categoryId, $subcategoryId, $status, $createdDate, $multipleIdSites)
    {
        ...
        if ($multipleIdSites) {
            $columns['idsite'] = -1;
            $columns['multiple_idsites'] = $this->serializeMultipleIdSites($multipleIdSites);
        }
    ...
    private function enrichRecords($records, $idSite)
    {
        ...
        foreach ($records as $record) {
            if ($idSite === 'all' && !empty($record['multiple_idsites'])) {
                ...
                $multipleIdSites = $record['multiple_idsites'] ? explode(',', $record['multiple_idsites']) : [];
                if (!in_array((int) $idSite, array_map('intval', $multipleIdSites), true)) {
                    continue;
                }
    ...
    private function enrichRecord($record, $idSite)
    {
        ...
        if ($record['idsite'] == -1) {
            $multipleIdSites = $record['multiple_idsites'] ? explode(',', $record['multiple_idsites']) : [];
            $isAllWebsitesRequest = Validator::isAllWebsitesRequest($idSite);
            $allowAllWebsitesRequest = $isAllWebsitesRequest && Piwik::hasUserSuperUserAccess();
            if ($idSite != -1 && !$allowAllWebsitesRequest && !in_array((int) $idSite, array_map('intval', $multipleIdSites), true)) {
                return [];
            }
            $record['idsite'] = $idSite;
        }
