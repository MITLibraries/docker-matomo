...
use Piwik\Plugins\CustomReports\ReportType\KpiCard;
...
class Controller extends \Piwik\Plugin\Controller
{
    ...
    public function previewReport()
    {
        ...
        /** @var array|null $report the event handler populates the report by reference */
    ...
    /**
     * Renders a KPI card for a single metric custom report (metric value, sparkline and
     * comparison with the previous period).
     * @return string
     * @throws \Exception
     */
    public function getSingleMetricView()
    {
        $this->checkSitePermission();
        $this->validator->checkReportViewPermission($this->idSite);

        $idCustomReport = Common::getRequestVar('idCustomReport', null, 'int');

        $model = StaticContainer::get('Piwik\Plugins\CustomReports\Model\CustomReportsModel');
        $report = $model->getCustomReport($this->idSite, $idCustomReport);

        if (empty($report) || $report['report_type'] !== KpiCard::ID) {
            throw new \Exception(Piwik::translate('CustomReports_ErrorReportDoesNotExist'));
        }

        $metric = reset($report['metrics']);

        $reportMetadata = \Piwik\API\Request::processRequest('API.getMetadata', array(
            'idSite' => $this->idSite,
            'apiModule' => 'CustomReports',
            'apiAction' => 'getCustomReport',
            'apiParameters' => array('idCustomReport' => $idCustomReport),
        ));
        $reportMetadata = reset($reportMetadata);

        $metricTranslations = array();
        $metricDocumentations = array();
        if (!empty($reportMetadata)) {
            $metricTranslations = array_merge(
                !empty($reportMetadata['metrics']) ? $reportMetadata['metrics'] : array(),
                !empty($reportMetadata['processedMetrics']) ? $reportMetadata['processedMetrics'] : array()
            );
            if (!empty($reportMetadata['metricsDocumentation'])) {
                $metricDocumentations = $reportMetadata['metricsDocumentation'];
            }
        }

        $output = '<div vue-entry="CustomReports.KpiCardView"
            id-custom-report="' . $this->getVueEntryValue($idCustomReport) . '"
            metric="' . $this->getVueEntryValue($metric) . '"
            metric-translation="' . $this->getVueEntryValue(!empty($metricTranslations[$metric]) ? $metricTranslations[$metric] : $metric) . '"
            metric-documentation="' . $this->getVueEntryValue(!empty($metricDocumentations[$metric]) ? $metricDocumentations[$metric] : '') . '"
            lower-is-better="' . $this->getVueEntryValue(\Piwik\Metrics::isLowerValueBetter($metric)) . '"
        ></div>';

        $isWidget = Common::getRequestVar('widget', 0, 'int');
        $showTitle = Common::getRequestVar('showtitle', 0, 'int');

        if (!$isWidget && $showTitle) {
            // on reporting pages (which request showtitle=1) the widget needs to render the same
            // card and title markup as regular reports do (see _dataTable.twig); on dashboards
            // and in widgetize the surrounding widget provides the chrome
            $editUrl = '';
            if ($this->validator->canWrite($this->idSite)) {
            ...
            $title = '<h2 class="card-title" vue-entry="CoreHome.EnrichedHeadline"';
            if (!empty($editUrl)) {
                $title .= ' edit-url="' . Common::sanitizeInputValue($editUrl) . '"';
            }
            if (!empty($report['description'])) {
                $title .= ' inline-help="' . Common::sanitizeInputValue($report['description']) . '"';
            }
            $title .= '>' . Common::sanitizeInputValue($report['name']) . '</h2>';

            $output = '<div class="card"><div class="card-content">' . $title . $output . '</div></div>';
        }
    ...
    {
        return Common::sanitizeInputValue(json_encode($value));
    }
}
