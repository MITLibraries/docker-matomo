      ...
      <div>
        <div name="name">
          <Field
            ...
            :inline-help="translate('CustomReports_ReportNameHelpText')"
          >
          </Field>
        </div>
        <div name="description">
          <Field
            ...
            :inline-help="getDescriptionHelpText"
          >
          </Field>
        </div>
        ...
        <div class="form-group row">
          ...
          <div class="col s12 m6">
            <div class="form-help">
              <span class="inline-help">{{ translate(isSingleMetricType
                ? 'CustomReports_ReportSingleMetricHelp'
                : 'CustomReports_ReportMetricsHelp') }}</span>
            </div>
          </div>
        </div>
        <div
...
const DIMENSIONS_ALLOWED_TO_SHARE_SQL_SEGMENT = [
  ['UserCountry.Continent', 'UserCountry.Country'],
];
...
function canShareSqlSegment(selectedDimensionIds: string[], dimensionId: string) {
  return DIMENSIONS_ALLOWED_TO_SHARE_SQL_SEGMENT.some((group) => (
    group.includes(dimensionId)
    && selectedDimensionIds.some((selectedDimensionId) => (
      selectedDimensionId !== dimensionId && group.includes(selectedDimensionId)
    ))
  ));
}
...
export default defineComponent({
  ...
  computed: {
    ...
    isSingleMetricType() {
      return this.report?.report_type === 'kpicard';
    },
    typeNeedsDimensions() {
      return this.report?.report_type !== 'evolution' && this.report?.report_type !== 'kpicard';
    },
    visibleMetrics() {
      const reportMetrics = this.report?.metrics || [];
    ...
    dimensions() {
      ...
      for (let j = 0; j < result.length; j += 1) {
        ...
        if (dim.sqlSegment
          ...
          && !canShareSqlSegment(this.report.dimensions, dim.key)
    ...
    getDescriptionHelpText() {
      const linkString = externalLink('https://matomo.org/faq/custom-reports/how-to-add-an-information-icon-to-the-title-of-a-custom-report/');
      return translate('CustomReports_ReportDescriptionHelpText', linkString, '</a>');
    },
