...
class API extends \Piwik\Plugin\API
{
     ...
     * @param string $reportType Report type to create, for example "table", "evolution" or "kpicard".
    ...
    public function addCustomReport($idSite, $name, $reportType, $metricIds, $categoryId = false, $dimensionIds = array(), $subcategoryId = false, $description = '', $segmentFilter = '', $multipleIdSites = [])
    {
        if (!empty($multipleIdSites) && $idSite != 'all' && $idSite != '0') {
            $multipleIdSites = $this->normalizeMultipleIdSites($multipleIdSites);
            foreach ($multipleIdSites as $multipleIdSite) {
                $this->validator->checkWritePermission($multipleIdSite);
            }
            if (!in_array((int) $idSite, $multipleIdSites, true)) {
     ...
     * @param string $name The report name.
     * @param string $reportType Report type to store, for example "table", "evolution" or "kpicard".
     * @param string[] $metricIds Metric IDs to include in the report.
     * @param string|false $categoryId Category ID to place the report in. Uses the default custom reports category when omitted.
     * @param string[] $dimensionIds Dimension IDs to include in the report.
     * @param string|false $subcategoryId Subcategory ID to place the report in. Creates a new reporting page when omitted.
    ...
    public function updateCustomReport(
        ...
        foreach ($this->getCurrentOwnerSites($report) as $currentSite) {
            $this->validator->checkWritePermission($currentSite);
        }
    ...
    /**
     * Normalize submitted website IDs to unique positive integers, rejecting any value
     * that is not an exact integer (e.g. padded, signed, decimal or exponent forms).
     */
    private function normalizeMultipleIdSites(array $multipleIdSites): array
    {
        $normalized = [];
        foreach ($multipleIdSites as $candidate) {
            $id = (int) $candidate;
            if ($id > 0 && (string) $id === (string) $candidate) {
                $normalized[] = $id;
            } else {
                throw new \Exception(Piwik::translate('CustomReports_ErrorInvalidIdSite', [$candidate]));
            }
        }
    ...
    private function scheduleReArchiving($idSite, $multipleIdSites, array $report, ?Date $startDate): void
    {
        ...
        if (ReportType::factory($report['report_type'])->usesNumericRecords()) {
     ...
     * @return int[]
     */
    private function getCurrentOwnerSites(array $report): array
    {
        $currentSites = !empty($report['multiple_idsites'])
            ? explode(',', $report['multiple_idsites'])
            : [$report['idsite']];

        $currentSites = array_filter($currentSites, static function ($site) {
            return $site !== '' && $site !== null && $site !== false;
        });

        return array_values(array_map('intval', $currentSites));
    }
