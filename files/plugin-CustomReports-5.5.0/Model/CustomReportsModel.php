...
class CustomReportsModel
{
    ...
    {
        $this->forceValuesAllowedByReportType($reportType, $dimensions, $metrics);
    ...
    /**
     * Removes values a report type does not support: dimensions for types that don't need them and
     * metrics exceeding the type's maximum (only the first ones are kept).
     */
    private function forceValuesAllowedByReportType($reportType, &$dimensions, &$metrics)
    {
        try {
            $type = \Piwik\Plugins\CustomReports\ReportType\ReportType::factory($reportType);
        } catch (Exception $e) {
            return; // an invalid report type will be reported by validateReportValues()
        }

        if (!$type->needsDimensions()) {
            $dimensions = array();
        }

        $maxMetrics = $type->getMaxMetrics();
        if ($maxMetrics !== null && is_array($metrics) && count($metrics) > $maxMetrics) {
            $metrics = array_slice(array_values($metrics), 0, $maxMetrics);
        }
    }
    ...
    public function updateCustomReport($idSite, $idCustomReport, $name, $description, $reportType, $dimensions, $metrics, $segmentFilter, $categoryId, $subcategoryId, $updatedDate, $subCategoryReportIds, $multipleIdSites)
    {
        ...
        if (!empty($multipleIdSites)) {
            $columns['idsite'] = -1;
            $columns['multiple_idsites'] = $this->dao->serializeMultipleIdSites($multipleIdSites);
        }
    ...
    private function validateReportValues($idSite, $name, $description, $reportType, $dimensions, $metrics, $segmentFilter, $categoryId, $subcategoryId, $subCategoryReportIds = [], $multipleIdSites = [])
    {
        ...
        $maxMetrics = $type->getMaxMetrics();
        if ($maxMetrics !== null && !empty($metrics) && is_array($metrics) && count($metrics) > $maxMetrics) {
            throw new Exception(Piwik::translate('CustomReports_ErrorTooManyMetrics', $maxMetrics));
        }
