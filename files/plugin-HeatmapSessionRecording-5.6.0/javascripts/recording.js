...
function HsrRecordingIframe (url) {
    ...
    this.sanitizeSelector = function (selector) {
    ...
    this.getIframeSandboxValue = function (nodeOrUrl) {
        ...
        if (nodeOrUrl) {
            if (typeof nodeOrUrl === 'string') {
                source = nodeOrUrl;
            } else if (nodeOrUrl.src) {
                source = nodeOrUrl.src;
            } else if (nodeOrUrl.getAttribute) {
                source = nodeOrUrl.getAttribute('src') || '';
            }
        }

        if (source && youtubeRegex.test(String(source).toLowerCase())) {
    ...
    this.restoreIframeSandboxPolicy = function (node) {
        if (!node || node.nodeType !== Node.ELEMENT_NODE) {
            return;
        }

        if (node.tagName === 'IFRAME' && !node.getAttribute('sandbox')) {
            node.setAttribute('sandbox', this.getIframeSandboxValue(node));
        }

        if (node.querySelectorAll) {
            var frames = node.querySelectorAll('iframe:not([sandbox])');
            for (var i = 0; i < frames.length; i++) {
                frames[i].setAttribute('sandbox', this.getIframeSandboxValue(frames[i]));
            }
        }
    };

    this.isBlockedMetaAttributeMutation = function (node, name, value) {
        if (!node || node.tagName !== 'META' || !name) {
            return false;
        }

        var nameLower = String(name).trim().toLowerCase();

        if (nameLower === 'http-equiv' && value) {
            var httpEquiv = String(value).trim().toLowerCase();
            return httpEquiv === 'content-security-policy' || httpEquiv === 'refresh';
        }

        if (nameLower === 'name' && value) {
            var metaName = String(value).trim().toLowerCase();
            return metaName === 'csrf-token' || metaName === 'referrer';
        }
        ...
        this.mirror = new TreeMirror(document, {
            createElement: function(tagName, data) {
                ...
                if (tagName === 'script') {
                    ...
                    if (tagName === 'iframe' && data && 'src' in data.attributes && data.attributes.src.indexOf('google.com/recaptcha') !== -1) {
                        var element = document.createElement('NO_SCRIPT');
                        element.style.display = 'none';
                        return element;
                    }
                    if (isLinkHrefAttr && data && 'href' in data.attributes && data.attributes['href']) {
                    ...
                    if (isLinkHrefAttr && data && 'data-matomo-href' in data.attributes && data.attributes['data-matomo-href']) {
                    ...
                    if (element.tagName === 'IFRAME') {
                        element.setAttribute('sandbox', self.getIframeSandboxValue(data && data.attributes ? data.attributes.src : ''));
                    }
                    ...
                    if (data && typeof data.attributes === 'object') {
                        if ('http-equiv' in data.attributes && data.attributes['http-equiv']) {
                            var httpEquiv = String(data.attributes['http-equiv']).trim().toLowerCase();
                        ...
                        if ('name' in data.attributes && data.attributes['name']) {
                            var metaName = String(data.attributes['name']).trim().toLowerCase();
            ...
            removeAttribute: function (node, name) {
                ...
                if (nameLower === 'sandbox' || nameLower === 'referrerpolicy') {
                    // do not remove our policy
                    return node;
                }
            },
            setAttribute: function(node, name, value) {
                ...
                if (node.tagName === 'META') {
                    if (self.isBlockedMetaAttributeMutation(node, name, value)) {
                        // the createElement delegate only sees the attributes an element is created with,
                        // so we also need to prevent a later mutation from turning a harmless meta element
                ...
                if (node.tagName === 'IFRAME') {
                    node.setAttribute('sandbox', self.getIframeSandboxValue(value && nameLower === 'src' ? value : node));
    ...
    this.applyMutation = function (event) {
        if (event) {
            ...
            // the tree mirror does not consult our delegate when it moves nodes, and re-inserting
            // an iframe reloads it, so we restore the sandbox only on the moved subtree
            var addedOrMoved = event.adOrMo || [];
            for (var i = 0; i < addedOrMoved.length; i++) {
                var movedNode = this.mirror.getNode ? this.mirror.getNode(addedOrMoved[i].id) : null;
                this.restoreIframeSandboxPolicy(movedNode);
            }
        }
    };
