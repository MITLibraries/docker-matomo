...
class HsrStore<T extends Heatmap|SessionRecording> {
  ...
  readonly statusOptions = computed(() => {
    const options = [
      ...
      { key: 'onhold', value: translate('HeatmapSessionRecording_StatusOnHold') },
    ];
  ...
  createOrUpdateHsr(hsr: Heatmap|SessionRecording, method: string): Promise<{
    ...
    const params = {
      ...
      autoRepeat: (hsr as Heatmap).auto_repeat ? 1 : 0,
      description: (hsr as SessionRecording).description !== undefined
        ? ((hsr as SessionRecording).description as string).trim()
        : undefined,
      method,
      name: hsr.name.trim(),
    };
