...
<template>
  <div class="sessionRecordingList">
    <ContentBlock :content-title="translate('HeatmapSessionRecording_ManageSessionRecordings')">
      ...
      <table v-content-table v-tooltips>
        <thead>
          <tr>
            ...
            <th class="autoRepeat">
              {{ translate('HeatmapSessionRecording_AutoRepeatSessionRecordingColumn') }}
              <span
                class="icon-info"
                :title="translate('HeatmapSessionRecording_AutoRepeatSessionRecordingColumnHelp')"
              ></span>
            </th>
        ...
        <tbody>
          ...
          >
            ...
            <td class="status status-onhold" v-else-if="hsr.status === 'onhold'">
              {{ translate('HeatmapSessionRecording_StatusOnHold') }}
              <span class="icon icon-help" :title="onHoldReason(hsr)"></span>
            </td>
            <td class="status" v-else>{{ ucfirst(hsr.status) }}</td>
            <td class="autoRepeat">
              {{ hsr.auto_repeat ? translate('General_Yes') : translate('General_No') }}
            </td>
            <td class="action">
              <a
              ...
              <a
                ...
                v-if="hsr.status !== 'ended' && hsr.status !== 'onhold'"
    ...
    >
      ...
      <p
        v-if="endingHsrRepeats"
        style="text-align: center;"
      >
        {{ translate('HeatmapSessionRecording_EndSessionRecordingRepeatNotice') }}
      </p>
      <input
        role="yes"
        type="button"
...
export default defineComponent({
  ...
  methods: {
    ...
    onHoldReason(hsr: SessionRecording) {
      return translate(
        'HeatmapSessionRecording_StatusOnHoldHelpSessionRecording',
        hsr.scheduled_date_pretty || '',
      );
    },
