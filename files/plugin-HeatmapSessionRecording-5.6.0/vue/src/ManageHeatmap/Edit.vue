      ...
      <div>
        <div name="name">
          <Field
            ...
            :maxlength="255"
            :placeholder="translate('HeatmapSessionRecording_FieldNamePlaceholderV2')"
            :inline-help="translate('HeatmapSessionRecording_HeatmapNameHelpText')"
          >
          </Field>
        </div>
        <div name="description">
          <Field
            uicontrol="textarea"
            name="description"
            :model-value="siteHsr.description"
            @update:model-value="siteHsr.description = $event; setValueHasChanged()"
            :title="translate('HeatmapSessionRecording_DescriptionOptional')"
            :maxlength="255"
            :rows="3"
            :ui-control-attributes="{ class: 'compact-textarea' }"
            :placeholder="translate('HeatmapSessionRecording_FieldDescriptionPlaceholder')"
            :inline-help="translate('HeatmapSessionRecording_HeatmapDescriptionHelpText')"
          >
          </Field>
        </div>
        <div name="autoRepeat">
          <Field
            uicontrol="checkbox"
            name="auto_repeat"
            :title="translate('HeatmapSessionRecording_AutoRepeatHeatmap')"
            :model-value="siteHsr.auto_repeat"
            @update:model-value="siteHsr.auto_repeat = $event; setValueHasChanged()"
          >
            <template v-slot:inline-help>
              <div class="inline-help-node">
                <span v-html="$sanitize(autoRepeatHelp)"/>
              </div>
            </template>
          </Field>
        </div>
        <div class="form-group row">
          <div class="col s12">
            <h3 style="font-weight: 600">
              {{ translate('HeatmapSessionRecording_TargetPage') }}
            </h3>
          </div>
          <div
            class="col s12 m6"
          ...
          >
            <div v-if="!showAdvancedTargeting">
              <div
                class="matchPageRules simple"
                v-if="siteHsr.match_page_rules && siteHsr.match_page_rules.length"
              >
                <HsrUrlTarget
                  :model-value="siteHsr.match_page_rules[0]"
                  @update:model-value="setMatchPageRule($event, 0)"
              ...
              <p
                ...
                v-html="$sanitize(useAdvancedTargetingText)"
                @click="onTargetingToggleClick"
              />
            </div>
            <div v-if="showAdvancedTargeting">
              <p
                class="targetingToggle"
                v-html="$sanitize(useSimpleTargetingText)"
                @click="onTargetingToggleClick"
              />
              <div
          ...
          <div class="col s12 m6">
            <div class="form-help">
              <span class="inline-help">
                <span v-html="$sanitize(targetHelpText)" />
                <div>
        ...
        <div class="form-group row advancedSettingsToggle">
          <div class="col s12">
            <h3>{{ translate('HeatmapSessionRecording_AdvancedOptions') }}</h3>
            ...
            <p>
              <a
                ...
                v-show="!showAdvancedView"
                @click.prevent="showAdvancedView = true"
              >{{ translate('HeatmapSessionRecording_ShowAdvancedOptions') }}
                <span class="icon-chevron-down"></span></a>
              <a
                href=""
                class="hideAdvancedSettings"
                v-show="showAdvancedView"
                @click.prevent="showAdvancedView = false"
              >{{ translate('HeatmapSessionRecording_HideAdvancedOptions') }}
                <span class="icon-chevron-up"></span></a>
            </p>
          </div>
        </div>
        <div v-show="showAdvancedView">
          <div class="heatmapAdvancedSection">
            <h3>
              {{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionRecordingSettings') }}
            </h3>
            <p class="sectionDescription">
              {{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionRecordingSettingsHelp') }}
            </p>
            <div name="sampleRate">
              <Field
                uicontrol="select"
          ...
          <div class="heatmapAdvancedSection">
            <h3>{{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionPageSnapshot') }}</h3>
            <p class="sectionDescription">
              {{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionPageSnapshotHelp') }}
            </p>
          ...
          <div class="heatmapAdvancedSection">
            <h3>{{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionContentMasking') }}</h3>
            <p class="sectionDescription">
              {{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionContentMaskingHelp') }}
            </p>
          ...
          <div class="heatmapAdvancedSection">
            <h3>
              {{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionDeviceBreakpoints') }}
            </h3>
            <p class="sectionDescription">
              {{ translate('HeatmapSessionRecording_HeatmapAdvancedSectionDeviceBreakpointsHelp') }}
            </p>
            ...
            <div name="breakpointTablet">
              <Field
                ...
                :maxlength="4"
                :inline-help="breakpointTabletInlineHelp"
              >
              </Field>
            </div>
          </div>
...
import { isSimpleCompatible } from '../HsrUrlTarget/simpleTargeting';
...
export default defineComponent({
  ...
  methods: {
    ...
    onTargetingToggleClick(event: Event) {
  ...
  computed: {
    ...
    breakpointMobileInlineHelp() {
      return translate('HeatmapSessionRecording_BreakpointMobileManageHelp');
    },
    breakpointTabletInlineHelp() {
      const help1 = translate('HeatmapSessionRecording_BreakpointTabletManageHelp');
      const help2 = translate('HeatmapSessionRecording_BreakpointGeneralHelpManage');
      return `${help1} ${help2}`;
    },
    advancedOptionsSummary() {
      const sampleRate = NumberFormatter.formatNumber(
        Number.parseFloat(`${this.siteHsr.sample_rate ?? 0}`),
      );
      const sampleLimit = NumberFormatter.formatNumber(
        Number(this.siteHsr.sample_limit ?? 0),
      );
      const snapshotMode = this.siteHsr.capture_manually
        ? translate('HeatmapSessionRecording_ManualSnapshot')
        : translate('HeatmapSessionRecording_AutomaticSnapshot');
      const breakpointMobile = NumberFormatter.formatNumber(
        Number(this.siteHsr.breakpoint_mobile ?? 0),
      );
      const breakpointTablet = NumberFormatter.formatNumber(
        Number(this.siteHsr.breakpoint_tablet ?? 0),
      );

      return translate(
        'HeatmapSessionRecording_HeatmapAdvancedOptionsSummary',
        sampleRate,
        sampleLimit,
        snapshotMode,
        breakpointMobile,
    ...
    autoRepeatHelp() {
      ...
      return translate(
        'HeatmapSessionRecording_AutoRepeatHeatmapHelp',
    ...
    useAdvancedTargetingText() {
      return translate(
        'HeatmapSessionRecording_UseAdvancedTargeting',
        '<a href="" class="showAdvancedTargeting">',
        '</a>',
      );
    },
    useSimpleTargetingText() {
      return translate(
        'HeatmapSessionRecording_UseSimpleTargeting',
        '<a href="" class="showSimpleTargeting">',
        '</a>',
      );
    },
    targetHelpText() {
      const docUrl = 'https://matomo.org/faq/heatmap-session-recording/';
      const key = this.showAdvancedTargeting
        ? 'HeatmapSessionRecording_FieldIncludedTargetsHelpAdvanced'
        : 'HeatmapSessionRecording_FieldIncludedTargetsHelpSimple';
      return translate(key, externalLink(docUrl), '</a>');
    },
  },
});
</script>
