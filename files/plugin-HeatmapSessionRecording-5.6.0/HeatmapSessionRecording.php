 ...
 * Description: Visualise behaviour with Heatmaps and replay recorded visitor sessions to identify friction, validate design decisions, and improve conversion paths.
 * Author: InnoCraft
...
use Piwik\Plugins\HeatmapSessionRecording\Fake\HeatmapDataGenerator;
...
class HeatmapSessionRecording extends \Piwik\Plugin
{
    ...
    public const SESSION_TOKEN_EMBED_KEY = 'hsr.tokenEmbed';
    ...
    public function registerEvents()
    {
        return array(
            ...
            'VisitorGenerator.VisitsFake.trackVisit' => 'generateHeatmapData',
        );
    }

    /**
     * Adds fabricated heatmap data (clicks/moves/scrolls) to visits created by the VisitorGenerator plugin.
     *
     * For active heatmaps that already have a snapshot, selectors are derived from the stored
     * page_treemirror so the generated events align with the elements the heatmap renders. The request
     * is queued on the tracker's bulk actions (not sent directly), per the VisitorGenerator contract.
     *
     * @param \MatomoTracker $tracker
     ...
     */
    public function generateHeatmapData($tracker, $faker)
    {
        // only a portion of generated visits carry heatmap data, mimicking real-world sample rates
        if (!$faker->boolean(60)) {
            return;
        }

        $idSite = (int) $tracker->idSite;
        $generators = $this->getHeatmapGenerators($idSite);
        if (empty($generators)) {
            return;
        }

        $idSiteHsr = $faker->randomElement(array_keys($generators));

        // use the visit's tracked resolution as the viewport: `res` determines the heatmap
        // device group (LogHsr::getDeviceType()), so a mobile visit must not get a desktop viewport
        $screenWidth = !empty($tracker->width) ? (int) $tracker->width : null;
        $screenHeight = !empty($tracker->height) ? (int) $tracker->height : null;
        $visit = $generators[$idSiteHsr]->generateVisit($faker, $screenWidth, $screenHeight);
        if ($visit === null) {
            return;
        }

        $tracker->setCustomTrackingParameter('ca', '1');
        $tracker->setCustomTrackingParameter('pv_id', Common::getRandomString(6));
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_HSR_ID_VIEW, Common::getRandomString(6));
        // hsr_ids and hsr_ev are arrays by design: http_build_query() serializes them into the
        // request and the tracker endpoint requires arrays (RequestProcessor::afterRequestProcessed);
        // only the docblock of setCustomTrackingParameter() is narrower than what it supports
        // @phpstan-ignore argument.type
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_HSR_IDS, [$idSiteHsr]);
        // @phpstan-ignore argument.type
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_EVENTS, $visit['events']);
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_TOTAL_TIME, (string) $visit['total_time']);
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_VIEWPORT_WIDTH, (string) $visit['viewport_width']);
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_VIEWPORT_HEIGHT, (string) $visit['viewport_height']);
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_FOLD_Y_PERCENT, (string) $visit['fold']);
        $tracker->setCustomTrackingParameter(RequestProcessor::TRACKING_PARAM_SCROLL_MAX_PERCENTAGE, (string) $visit['scroll_max']);

        // queue with the visit's other requests; presence of hsr_vid makes the server skip the pageview/action
        $tracker->storedTrackingActions[] = $tracker->getUrlTrackPageView('Heatmap data (generated)');
        $tracker->clearCustomTrackingParameters();
    }

    /**
     * @param int $idSite
     * @return HeatmapDataGenerator[] indexed by idsitehsr (active heatmaps that have a snapshot)
     */
    private function getHeatmapGenerators($idSite)
    {
        $cache = Cache::getTransientCache();
        $cacheKey = 'HeatmapSessionRecording.heatmapDataGenerators.' . (int) $idSite;
        if ($cache->contains($cacheKey)) {
            return $cache->fetch($cacheKey);
        }

        $generators = [];
        $model = StaticContainer::get(SiteHsrModel::class);
        foreach ($model->getActiveHeatmapsWithPageTreemirror($idSite) as $heatmap) {
            if (empty($heatmap['page_treemirror'])) {
                continue;
            }
            $generator = new HeatmapDataGenerator($heatmap['page_treemirror']);
            if ($generator->hasTargets()) {
                $generators[(int) $heatmap['idsitehsr']] = $generator;
            }
        }

        $cache->save($cacheKey, $generators);
        return $generators;
    }
    ...
    public function addJsGlobalVariables()
    {
        ...
        if ($idSite > 0 && Piwik::isUserHasAdminAccess($idSite)) {
            echo 'piwik.heatmapAdminAccess = true;';
        } else {
            echo 'piwik.heatmapAdminAccess = false;';
        }
    }
    ...
    public function requiresTwoFactorAuthentication(&$requiresAuth, $module, $action, $parameters)
    {
        if (
            ...
            && $action === 'embedPage'
            && !empty($_SESSION[self::SESSION_TOKEN_EMBED_KEY])
        ) {
            $requiresAuth = false;
        }
    }
...
target="_blank" rel="noreferrer noopener"><span class="icon-play"></span> ' . $title . '</a><br />';
    ...
    {
        $this->getSiteHsrDao()->anonymizeLogin($userLogin);
    }
    ...
    public function getClientSideTranslationKeys(&$result)
    {
        ...
        $result[] = 'HeatmapSessionRecording_HeatmapSettings';
        $result[] = 'HeatmapSessionRecording_LoadingHeatmap';
        $result[] = 'HeatmapSessionRecording_EditHeatmap';
        $result[] = 'HeatmapSessionRecording_DeleteRetakeSnapshot';
        $result[] = 'HeatmapSessionRecording_DeleteRetakeSnapshotTitle';
        $result[] = 'HeatmapSessionRecording_DeleteRetakeSnapshotDescription';
        $result[] = 'HeatmapSessionRecording_DeleteRetakeAutomatically';
        $result[] = 'HeatmapSessionRecording_DeleteRetakeManually';
        $result[] = 'HeatmapSessionRecording_DeleteSnapshotTitle';
        $result[] = 'HeatmapSessionRecording_DeleteSnapshotDescription';
        $result[] = 'HeatmapSessionRecording_DeleteSnapshotConfirm';
        $result[] = 'HeatmapSessionRecording_EmptyStateNoInteractionsTitle';
        $result[] = 'HeatmapSessionRecording_EmptyStateNoInteractionsSubtitle';
        $result[] = 'HeatmapSessionRecording_EmptyStateSnapshotDeletedTitle';
        $result[] = 'HeatmapSessionRecording_EmptyStateSnapshotDeletedAutoSubtitle';
        $result[] = 'HeatmapSessionRecording_EmptyStateSnapshotDeletedManualSubtitle';
        $result[] = 'HeatmapSessionRecording_EmptyStateSnapshotDeletedPausedSubtitle';
        $result[] = 'HeatmapSessionRecording_EmptyStateSnapshotDeletedEndedSubtitle';
        $result[] = 'HeatmapSessionRecording_ReadTheGuide';
        ...
        $result[] = 'HeatmapSessionRecording_StatusOnHold';
        $result[] = 'HeatmapSessionRecording_StatusOnHoldHelp';
        $result[] = 'HeatmapSessionRecording_StatusOnHoldHelpSessionRecording';
        $result[] = 'HeatmapSessionRecording_RecordedHeatmapDocStatusOnHold';
        $result[] = 'HeatmapSessionRecording_RecordedHeatmapDocStatusOnHoldWithDate';
        ...
        $result[] = 'HeatmapSessionRecording_ReportNoLongerExists';
        ...
        $result[] = 'HeatmapSessionRecording_CustomWidth';
        $result[] = 'HeatmapSessionRecording_LegendLow';
        $result[] = 'HeatmapSessionRecording_LegendHigh';
        $result[] = 'HeatmapSessionRecording_Action';
        $result[] = 'HeatmapSessionRecording_ExportImage';
        $result[] = 'HeatmapSessionRecording_ExportImageError';
        $result[] = 'HeatmapSessionRecording_ExportImageInProgress';
        ...
        $result[] = 'HeatmapSessionRecording_AvgFold';
        ...
        $result[] = 'HeatmapSessionRecording_SessionNameHelpText';
        $result[] = 'HeatmapSessionRecording_HeatmapDescriptionHelpText';
        $result[] = 'HeatmapSessionRecording_DescriptionOptional';
        ...
        $result[] = 'HeatmapSessionRecording_ShowAdvancedOptions';
        $result[] = 'HeatmapSessionRecording_HideAdvancedOptions';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedOptionsSummary';
        $result[] = 'HeatmapSessionRecording_AutomaticSnapshot';
        $result[] = 'HeatmapSessionRecording_ManualSnapshot';
        ...
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionRecordingSettings';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionRecordingSettingsHelp';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionPageSnapshot';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionPageSnapshotHelp';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionContentMasking';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionContentMaskingHelp';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionDeviceBreakpoints';
        $result[] = 'HeatmapSessionRecording_HeatmapAdvancedSectionDeviceBreakpointsHelp';
        ...
        $result[] = 'HeatmapSessionRecording_BreakpointMobileManageHelp';
        $result[] = 'HeatmapSessionRecording_BreakpointTabletManageHelp';
        ...
        $result[] = 'HeatmapSessionRecording_FieldIncludedTargetsHelpSimple';
        $result[] = 'HeatmapSessionRecording_FieldIncludedTargetsHelpSimpleSessions';
        $result[] = 'HeatmapSessionRecording_FieldIncludedTargetsHelpAdvanced';
        $result[] = 'HeatmapSessionRecording_FieldIncludedTargetsHelpAdvancedSessions';
        $result[] = 'HeatmapSessionRecording_UseAdvancedTargeting';
        $result[] = 'HeatmapSessionRecording_UseSimpleTargeting';
        ...
        $result[] = 'HeatmapSessionRecording_FieldNamePlaceholderV2';
        $result[] = 'HeatmapSessionRecording_FieldDescriptionPlaceholder';
        $result[] = 'HeatmapSessionRecording_HeatmapNameHelpText';
        ...
        $result[] = 'HeatmapSessionRecording_EndHeatmapRepeatNotice';
        $result[] = 'HeatmapSessionRecording_EndSessionRecordingConfirm';
        $result[] = 'HeatmapSessionRecording_EndSessionRecordingRepeatNotice';
        ...
        $result[] = 'HeatmapSessionRecording_AutoRepeatHeatmap';
        $result[] = 'HeatmapSessionRecording_AutoRepeatHeatmapHelp';
        $result[] = 'HeatmapSessionRecording_AutoRepeatSessionRecording';
        $result[] = 'HeatmapSessionRecording_AutoRepeatSessionRecordingHelp';
        $result[] = 'HeatmapSessionRecording_AutoRepeatHeatmapColumn';
        $result[] = 'HeatmapSessionRecording_AutoRepeatHeatmapColumnHelp';
        $result[] = 'HeatmapSessionRecording_AutoRepeatSessionRecordingColumn';
        $result[] = 'HeatmapSessionRecording_AutoRepeatSessionRecordingColumnHelp';
        ...
        $result[] = 'HeatmapSessionRecording_Recordings';
        $result[] = 'HeatmapSessionRecording_WaitingForFirstRecording';
        $result[] = 'HeatmapSessionRecording_TopClickedElements';
        $result[] = 'HeatmapSessionRecording_NoClickedElements';
        $result[] = 'HeatmapSessionRecording_ScrollReach';
        $result[] = 'HeatmapSessionRecording_NoScrollData';
        $result[] = 'HeatmapSessionRecording_ScrollReachTop';
        $result[] = 'HeatmapSessionRecording_ScrollReachBottom';
        $result[] = 'HeatmapSessionRecording_TopClickedElementLabel';
        $result[] = 'HeatmapSessionRecording_ElementTypeArticle';
        $result[] = 'HeatmapSessionRecording_ElementTypeButton';
        $result[] = 'HeatmapSessionRecording_ElementTypeField';
        $result[] = 'HeatmapSessionRecording_ElementTypeFooter';
        $result[] = 'HeatmapSessionRecording_ElementTypeForm';
        $result[] = 'HeatmapSessionRecording_ElementTypeHeader';
        $result[] = 'HeatmapSessionRecording_ElementTypeHeading';
        $result[] = 'HeatmapSessionRecording_ElementTypeImage';
        $result[] = 'HeatmapSessionRecording_ElementTypeLink';
        $result[] = 'HeatmapSessionRecording_ElementTypeList';
        $result[] = 'HeatmapSessionRecording_ElementTypeMainContent';
        $result[] = 'HeatmapSessionRecording_ElementTypeNavigation';
        $result[] = 'HeatmapSessionRecording_ElementTypeSection';
        $result[] = 'HeatmapSessionRecording_ElementTypeSidebar';
