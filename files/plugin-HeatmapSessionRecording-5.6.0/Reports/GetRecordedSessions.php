        ...
        if (!empty($idSite) && !empty($idSiteHsr)) {
            ...
            if (!empty($recording)) {
                ...
                if (empty($idSubtable)) {
                    ...
                    } elseif ($recording['status'] == SiteHsrDao::STATUS_ONHOLD) {
                        if (!empty($recording['scheduled_date_pretty'])) {
                            $view->config->show_footer_message = Piwik::translate(
                                'HeatmapSessionRecording_RecordedSessionsDocStatusOnHoldWithDate',
                                $recording['scheduled_date_pretty']
                            );
                        } else {
                            $view->config->show_footer_message = Piwik::translate('HeatmapSessionRecording_RecordedSessionsDocStatusOnHold');
                        }
                    }
                }
            }
