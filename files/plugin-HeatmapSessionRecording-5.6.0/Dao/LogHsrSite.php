...
class LogHsrSite
{
    ...
    public function deleteNoLongerNeededRecords()
    {
        ...
        $sql = sprintf(
            'DELETE FROM %1$s WHERE %1$s.idsitehsr NOT IN (select site_hsr.idsitehsr from %2$s site_hsr where site_hsr.status = "%3$s" or site_hsr.status = "%4$s" or site_hsr.status = "%5$s" or site_hsr.status = "%6$s")',
            Common::prefixTable('log_hsr_site'),
            Common::prefixTable('site_hsr'),
            SiteHsrDao::STATUS_ACTIVE,
            SiteHsrDao::STATUS_ENDED,
            SiteHsrDao::STATUS_PAUSED,
