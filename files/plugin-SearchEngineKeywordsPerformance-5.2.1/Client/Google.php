...
class Google
{
    ...
    protected function configure(
        ...
        if (
            ...
            if (empty($uri)) {
                throw new \Exception(Piwik::translate('SearchEngineKeywordsPerformance_InvalidRedirectUriInClientConfigurationNew', [$expectedUri]));
            }
    ...
    /**
     * Request-scoped cache of the raw listSites() response (or the exception it threw),
     * keyed by a hash of the access token. Both testConfiguration() and getAvailableUrls()
     * hit the same endpoint, so this collapses the duplicate calls within a single request.
     *
     * @var array
     */
    private $listSitesCache = [];
    /**
     * Fetches (and memoizes for the current request) the raw listSites() response for the
     * given access token. A previously thrown exception is cached and re-thrown so callers
     * keep their existing error handling.
     *
     * @param string $accessToken
     * @return \Matomo\Dependencies\SearchEngineKeywordsPerformance\Google\Service\SearchConsole\SitesListResponse
     * @throws \Exception
     */
    protected function fetchSitesResponse(
        #[\SensitiveParameter]
        $accessToken
    ) {
        $key = sha1(serialize($accessToken));
        if (array_key_exists($key, $this->listSitesCache)) {
            if ($this->listSitesCache[$key] instanceof \Exception) {
                throw $this->listSitesCache[$key];
            }
            return $this->listSitesCache[$key];
        }
        try {
            ...
            $this->listSitesCache[$key] = $e;
            throw $e;
        }
        $this->listSitesCache[$key] = $response;
        return $response;
    }
    /**
    ...
    public function testConfiguration($accountId)
    {
        ...
        try {
            $this->fetchSitesResponse($accessToken);
        } catch (\Exception $e) {
            // re-throw with only the description so the raw API response is never shown to the user
            $this->handleServiceException($e);
            throw new UnknownAPIException($this->extractErrorDescription($e));
        }
        return \true;
    }
    /**
    ...
    public function getAvailableUrls($accountId, $removeUrlsWithoutAccess = \true)
    {
        ...
        $sites = [];
        try {
            $response = $this->fetchSitesResponse($accessToken);
        } catch (\Exception $e) {
            return $sites;
        }
    ...
    /**
     * Extracts the human readable description from a Google API exception, whose message is the
     * full JSON error response, so the raw response is never exposed to the user.
     *
     * @param \Exception $e
     * @return string
     */
    protected function extractErrorDescription($e)
    {
        $message = $e->getMessage();
        $error = json_decode($message, \true);
        if (is_array($error)) {
            ...
            if (!empty($error['error']) && is_string($error['error'])) {
                return $error['error'];
            }
