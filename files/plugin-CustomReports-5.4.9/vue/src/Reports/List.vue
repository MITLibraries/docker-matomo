        ...
        <tbody>
          ...
          >
            ...
            <td  :class="{
              ...
              />
              <span
                  v-if="isEntityDuplicatorAvailable &&
                  ((report.idsite && !isMultiSiteReport(report)) || report.allowedToEdit)"
              >
                <EntityDuplicatorAction
                    :actionFormData="{idCustomReport: report.idcustomreport}"
                  ...
                  'CustomReports_QuotaReachedForX',
                  translate('CustomReports_CustomReport'),
                  translate('CustomReports_CustomReports'),
                )"
                    :extraClasses="
                    ['customreport-duplicate-action', `customreport-${report.idcustomreport}`]"
                />
              </span>
              <a
  ...
  mounted() {
    ...
    if (this.entityDuplicatorStore) {
      ...
      adapter.onSuccessCallback = (
        ...
        return CustomReportsStore.reload().then(() => resolve());
      });
    }
  },
  methods: {
    ...
    checkIsAddingCustomReportsAllowed(): boolean {
      // Post event so that Billing,etc. can indicate whether adding another customreport is allowed
      const parameters = {
        isAllowed: true,
      };
      Matomo.postEvent('CustomReports.initAddCustomReport', parameters);
